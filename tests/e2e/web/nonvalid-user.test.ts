...
test.describe('Authentification', () => {
  ...
  test('Connexion', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Email professionnel' }).fill(email);
    await page.getByRole('button', { name: 'Continuer', exact: true }).click();
    await page.getByRole('textbox', { name: 'Renseignez votre mot de passe' }).fill(email);
    await page.getByRole('button', { name: 'S’identifier' }).click();
    await page
      .getByRole('button', { name: 'Commune de marseille - Mairie (choisir cette organisation)' })
      .click();
...
test.describe('Connecté mais non validés', () => {
  ...
  test('Les utilisateurs non validés ne peuvent pas acceder aux déclarations', async ({ page }) => {
    await page.goto('/declarations/2024/entreprise/1');
    await expect(page).toHaveURL('/validation');
  });
});
